...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        $id = (int)($_POST['id'] ?? 0);
        $pdo->prepare('DELETE FROM other_income WHERE id=? AND budget_id=?')->execute([$id, $budgetId]);
        $flash = 'Entry removed.';
    }
}
...
if ($filterSearch !== '') {
    $where[] = '(source LIKE ? OR notes LIKE ?)';
    $params[] = "%$filterSearch%";
    $params[] = "%$filterSearch%";
}
if ($filterYear !== null) {
...
$countStmt = $pdo->prepare("SELECT COUNT(*) FROM other_income WHERE $whereSql");
...
$stmt = $pdo->prepare("SELECT * FROM other_income WHERE $whereSql ORDER BY year DESC, id DESC LIMIT $perPage OFFSET $offset");
$stmt->execute($params);
